...
const styles = {
  ...
  rowLabel: { fontSize: 13, fontWeight: 500, color: 'var(--text-primary)', marginBottom: 2 },
  rowDesc: { fontSize: 11, color: 'var(--text-muted)', lineHeight: 1.4 },
  toggle: (active) => ({
  ...
  select: {
    ...
    fontSize: 12, cursor: 'pointer', outline: 'none', flexShrink: 0,
  },
  input: {
    background: 'var(--bg-secondary)', border: '1px solid var(--border)',
    borderRadius: 6, color: 'var(--text-primary)', padding: '7px 10px',
    fontSize: 12, outline: 'none', width: '100%', marginTop: 8,
    fontFamily: 'monospace', boxSizing: 'border-box',
  },
  dangerBtn: {
    padding: '7px 12px', background: 'rgba(255,68,68,0.1)',
  ...
  saveBtn: {
    ...
    cursor: 'pointer', flexShrink: 0, whiteSpace: 'nowrap', marginTop: 8,
  },
  savedBadge: {
    padding: '5px 10px', background: 'var(--accent-dim)',
    border: '1px solid rgba(29,185,84,0.3)', borderRadius: 6,
  ...
  statusDot: (ok) => ({
    ...
    background: ok ? 'var(--accent)' : 'var(--red)', marginRight: 5, flexShrink: 0,
  }),
  updateBtn: (variant) => ({
    padding: '7px 14px',
    ...
    fontSize: 12, fontWeight: variant === 'primary' ? 700 : 400,
    cursor: 'pointer', flexShrink: 0, whiteSpace: 'nowrap',
  }),
  progressBar: { height: 4, background: 'var(--border)', borderRadius: 2, overflow: 'hidden', marginTop: 8 },
  progressFill: (pct) => ({
    height: '100%', width: `${pct}%`, background: 'var(--accent)',
    borderRadius: 2, transition: 'width 0.3s ease',
  }),
  themeGrid: {
    display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, padding: '14px 16px',
  },
  themeCard: (active, accent) => ({
...
function UpdateSection({ appVersion }) {
  ...
  useEffect(() => {
    const cleanup = window.electronAPI.onUpdateStatus((data) => setUpdateState(data));
    return cleanup;
  }, []);

  const handleCheck = () => { setUpdateState({ status: 'checking' }); window.electronAPI.checkForUpdates(); };
  const handleDownload = () => { setUpdateState({ status: 'downloading', percent: 0 }); window.electronAPI.downloadUpdate(); };
  const handleInstall = () => window.electronAPI.installUpdate();

  const renderRight = () => {
    switch (updateState.status) {
      case 'checking':
        return <span style={{ fontSize: 12, color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
      ...
      case 'available':
        return <div style={{ display: 'flex', flexDirection: 'column', gap: 6, alignItems: 'flex-end', flexShrink: 0 }}>
      ...
      case 'downloading':
        return <div style={{ minWidth: 140, flexShrink: 0 }}>
          <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>{updateState.percent || 0}% …</span>
          <div style={styles.progressBar}><div style={styles.progressFill(updateState.percent || 0)} /></div>
        </div>;
      case 'downloaded':
        return <div style={{ display: 'flex', flexDirection: 'column', gap: 6, alignItems: 'flex-end', flexShrink: 0 }}>
      ...
      default:
        return <button style={styles.updateBtn('default')} onClick={handleCheck}
...
// ─── Theme Section ────────────────────────────────────────────────────────────
const THEME_LIST = [
  { id: 'dark',     label: 'Dark',    accent: '#1DB954' },
  { id: 'light',    label: 'Light',   accent: '#1DB954' },
  { id: 'midnight', label: 'Midnight',accent: '#7c6af7' },
  { id: 'rose',     label: 'Rose',    accent: '#e84393' },
  { id: 'ocean',    label: 'Ocean',   accent: '#00b4d8' },
  { id: 'sunset',   label: 'Sunset',  accent: '#ff7b00' },
];

function ThemeSection({ currentTheme }) {
  return (
    <div style={styles.section}>
      <div style={styles.sectionHeader}>🎨 Theme</div>
      <div style={styles.themeGrid}>
        {THEME_LIST.map((t) => (
          <div key={t.id} style={styles.themeCard(currentTheme === t.id, t.accent)}>
            <div style={styles.themeDot(t.accent)} />
            <div style={{ fontSize: 11, color: currentTheme === t.id ? 'var(--accent)' : 'var(--text-muted)', fontWeight: currentTheme === t.id ? 600 : 400 }}>
              {t.label}
            </div>
            {currentTheme === t.id && (
              <div style={{ fontSize: 10, color: 'var(--accent)' }}>✓ Aktiv</div>
            )}
          </div>
        ))}
      </div>
      <div style={styles.infoBox}>
        🎨 Themes werden auf <span
          style={{ color: 'var(--accent)', cursor: 'pointer', textDecoration: 'underline' }}
          onClick={() => window.electronAPI.openExternal('https://mus-iq.com/settings')}>
          mus-iq.com
        </span> verwaltet und synchronisieren sich automatisch in die App.
      </div>
    </div>
  );
}
...
// ─── Main ─────────────────────────────────────────────────────────────────────
export default function SettingsPage({ session, onLogout, currentTheme = 'dark' }) {
  ...
  return (
    <div style={styles.page} className="fade-in">
      ...
      <div style={styles.section}>
        ...
        <div style={styles.row}>
          <div style={styles.rowLeft}>
            <div style={styles.rowLabel}>Echtzeit-Tracking</div>
            <div style={styles.rowDesc}>Song-Wechsel sofort erkennen (1s Polling) – empfohlen</div>
          </div>
          <Toggle value={settings.realtimeTracking !== false} onChange={(v) => updateSetting('realtimeTracking', v)} />
        </div>
        <div style={{ ...styles.row, opacity: settings.realtimeTracking !== false ? 0.4 : 1, pointerEvents: settings.realtimeTracking !== false ? 'none' : 'auto' }}>
          <div style={styles.rowLeft}>
            <div style={styles.rowLabel}>Tracking-Interval</div>
            <div style={styles.rowDesc}>Nur aktiv wenn Echtzeit-Tracking deaktiviert</div>
          </div>
          <select style={styles.select} value={settings.trackingInterval}
      ...
      {/* Theme */}
      <ThemeSection currentTheme={currentTheme} />
        ...
        <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--border)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
            ...
            <div style={{ fontSize: 11, flexShrink: 0 }}>
              <span style={styles.statusDot(hasClientId)} />{hasClientId ? 'Eingetragen' : 'Fehlt'}
            </div>
          </div>
          ...
          <input style={styles.input} placeholder="z.B. 1234567890123456789"
            value={clientId} onChange={(e) => setClientId(e.target.value)}
            onFocus={(e) => { e.target.style.borderColor = 'var(--accent)'; }}
            onBlur={(e) => { e.target.style.borderColor = 'var(--border)'; }} />
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 8 }}>
            <button style={styles.saveBtn} onClick={saveClientId}>Speichern & Verbinden</button>
